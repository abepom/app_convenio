...
const ListarAtendimento = (props) => {
  ...
  const getConsulta = async (mes, ano) => {
    setLoad(true)
    const dados = await api.get('/consultarAtendimentos', { params: { id_gds: props.navigation.state.params.id_gds, mes, ano } })
            ...
            renderItem={({ item }) => {
              ...
              return (<View style={{ elevation: 2, borderRadius: 4, marginBottom: 10, backgroundColor: `white`, paddingVertical: 10, paddingHorizontal: 5 }}>
                ...
                <Text style={{ fontWeight: "bold" }}>Matricula:
                                    <Text style={{ fontWeight: "100" }}> {item.Nr_Cartao_Abepom}</Text>
                </Text>
