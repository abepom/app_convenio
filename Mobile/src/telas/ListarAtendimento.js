    ...
    (props) => {
      ...
      const getConsulta = async (mesSelecionado, anoSelecionado) => {
        try {
          ...
          setLoad(true)
          const dados = await api.get('/consultarAtendimentos', { params: { id_gds, mes: mesSelecionado, ano: anoSelecionado } })
        ...
        <>
            ...
            <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
              {conteudoModal ? (<>
                ...
                <Text style={{ fontWeight: "100" }}> {conteudoModal.Nr_Cartao_Abepom.substr(0, 6)}</Text>
                  ...
                  <Text style={{ fontWeight: "100" }}> {formatCurrency.format(conteudoModal.CIU_Valor, { code: 'BRL' })}</Text>
                    </Text>
                ...
                <View style={{ flexDirection: "row", justifyContent: 'space-between' }}>
                  <TouchableOpacity
                    onPress={() => {
                      ...
                      excluirVenda(conteudoModal.id_CIU).then((a) => console.log(a, 'retorno'))
                      getConsulta(mes, ano)
                    }}
                    style={{
                    ...
                    return (
                      <TouchableOpacity onPress={() => {
                        ...
                        <View style={{ elevation: 2, borderRadius: 4, marginBottom: 10, backgroundColor: `white`, paddingVertical: 10, paddingHorizontal: 5 }}>
                          ...
                          <View style={{ flexDirection: "row", justifyContent: 'space-between' }}>
                            <Text style={{ fontWeight: "bold" }}>Valor:
                                <Text style={{ fontWeight: "100" }}> {formatCurrency.format(item.CIU_Valor, { code: 'BRL' })}</Text>
                            </Text>
                            <Text style={{ fontWeight: "bold" }}>Data:
          ...
          <View style={{ flexDirection: "row", alignContent: "space-between", width: '100%' }} >
            ...
            <View style={{ width: '50%', alignItems: "center" }}>
              ...
              <Text style={{ fontWeight: "bold", color: primary, fontSize: 20 }}>{vendas ? formatCurrency.format((vendas.reduce((total, valor) => total + Number(valor.CIU_Valor), 0)), { code: 'BRL' }) : 'R$ 0,00'}</Text>
            </View>
          </View>
        </>
      )
